...
SKU_VARIANT_CACHE: Dict[str, Optional[int]] = {
    "HS-GR-06": 52022421324086,   # Hair Growth Shampoo 3-month supply
    "HC-GR-03": 51683644178742,   # Hair Growth Conditioner 3-month supply
    "HB-GR-03": 51819958075702,   # Daily Hair Booster 3-month supply
    "PPP-01":   52762993492278,   # Porch Pirate Protection
}
...
def build_shopify_order(row: Dict, variant_map: Dict[str, int], date_str: str) -> Dict:
    ...
    oto_category = row["oto_category"]
    sku_info = OTO_SKU_MAP.get(oto_category, {})
    sku = sku_info.get("sku", row.get("shopify_sku", "UNKNOWN"))
    price = str(sku_info.get("price", row.get("shopify_price", 0)))
    title = sku_info.get("title", row.get("product_name", oto_category))
    ...
    # Add variant_id if known (preferred over title)
